...
<div class="container-fluid" style="padding: 30px;">
    ...
    <form action="{{ route('kepbupati.update', $encryptedId) }}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
        @method('PUT')

        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
            <div>
                <h2 style="font-weight: 700; color: #1e293b; margin: 0;">Edit Keputusan Bupati</h2>
                <p style="color: #64748b; margin: 0;">Memperbarui data: <strong>{{ $kepbupati->nomor }}</strong></p>
            </div>
            <div style="display:flex; gap:10px;">
                <a href="{{ route('kepbupati.index') }}" style="background:#f1f5f9; color:#475569; border-radius:12px; padding:12px 25px; text-decoration:none; font-weight:600; display:flex; align-items:center;">Batal</a>
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 25px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
                <div style="background: white; border-radius: 20px; padding: 25px; border: 1px solid #f1f5f9; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                    ...
                    <div style="margin-bottom: 20px;">
                        <label style="display:block; margin-bottom:8px; font-weight:600;">Nomor Keputusan <span class="req">*</span></label>
                        <input type="text" name="nomor" value="{{ old('nomor', $kepbupati->nomor) }}"
                    ...
                    <div style="margin-bottom: 20px;">
                        <label style="display:block; margin-bottom:8px; font-weight:600;">Judul Keputusan <span class="req">*</span></label>
                        <input type="text" name="judul" value="{{ old('judul', $kepbupati->judul) }}"
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        @if($kepbupati->file_pdf)
                            <div style="margin-bottom:10px; padding:10px; background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; font-size:13px; display:flex; align-items:center; gap:8px;">
                                ...
                                <span style="color:#166534; font-weight:600;">{{ basename($kepbupati->file_pdf) }}</span>
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" id="editor">{{ old('keterangan', $kepbupati->keterangan) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div style="background: white; border-radius: 20px; padding: 25px; border: 1px solid #f1f5f9; height: fit-content; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <input type="number" name="tahun" value="{{ old('tahun', $kepbupati->tahun) }}"
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <select name="status" class="{{ $errors->has('status') ? 'is-err' : '' }}"
                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; box-sizing:border-box;">
                        <option value="BERLAKU"       {{ old('status', $kepbupati->status) == 'BERLAKU'       ? 'selected' : '' }}>BERLAKU</option>
                        <option value="TIDAK BERLAKU" {{ old('status', $kepbupati->status) == 'TIDAK BERLAKU' ? 'selected' : '' }}>TIDAK BERLAKU</option>
                    </select>
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <input type="text" name="tempat_penetapan" value="{{ old('tempat_penetapan', $kepbupati->tempat_penetapan) }}"
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', $kepbupati->tanggal ? $kepbupati->tanggal->format('Y-m-d') : '') }}"
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <input type="text" name="sumber" value="{{ old('sumber', $kepbupati->sumber) }}"
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <input type="text" name="subjek" value="{{ old('subjek', $kepbupati->subjek) }}"
                ...
                <div style="margin-bottom:15px;">
                    ...
                    <input type="text" name="lokasi" value="{{ old('lokasi', $kepbupati->lokasi) }}"
                        style="width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; box-sizing:border-box;">
                </div>

                <div style="margin-bottom:5px;">
                    <label style="display:block; margin-bottom:8px; font-weight:600;">Bidang Hukum <span class="req">*</span></label>
                    <input type="text" name="bidang_hukum" value="{{ old('bidang_hukum', $kepbupati->bidang_hukum) }}"
