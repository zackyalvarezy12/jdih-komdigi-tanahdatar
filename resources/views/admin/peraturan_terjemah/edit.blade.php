...
<div style="padding: 40px; background-color: #f8fafc; min-height: 100vh; font-family: 'Inter', sans-serif;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            <h2 style="font-weight: 700; color: #1e293b; margin: 0; font-size: 26px; letter-spacing: -0.5px;">Edit Peraturan Terjemah</h2>
            <p style="color: #64748b; margin: 5px 0 0 0; font-size: 15px;">Sesuaikan informasi dokumen hukum yang telah diterjemahkan.</p>
        </div>
    ...
    <div style="background: white; border-radius: 20px; border: 1px solid #e2e8f0; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.04); overflow: hidden; max-width: 1000px; margin: 0 auto;">
        ...
        <div style="background: #f8fafc; padding: 20px 30px; border-bottom: 1px solid #e2e8f0;">
            <span style="background: #dcfce7; color: #166534; padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 700; text-transform: uppercase;">Form Pembaruan Data</span>
        </div>

        <form id="mainForm" action="{{ route('peraturan-terjemah.update', $item->id) }}" method="POST" enctype="multipart/form-data" style="padding: 40px;" novalidate>
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 25px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="type_dokumen" id="type_dokumen" value="{{ old('type_dokumen', $item->type_dokumen) }}" 
                           ...
                           placeholder="Contoh: Peraturan Bupati"
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $item->tahun) }}"
                           style="width: 100%; padding: 14px; border: 1.5px solid #e2e8f0; border-radius: 12px; outline: none; transition: 0.3s; font-size: 14px; box-sizing: border-box;"
                           onfocus="clearError(this)" 
                           onblur="validateField(this)">
            ...
            <div style="margin-bottom: 25px;">
                ...
                <textarea name="judul" id="judul" rows="4" 
                          ...
                          onfocus="clearError(this)" 
                          onblur="validateField(this)">{{ old('judul', $item->judul) }}</textarea>
            ...
            <div style="margin-bottom: 30px;">
                ...
                <input type="text" name="teu_pengarang" id="teu_pengarang" value="{{ old('teu_pengarang', $item->teu_pengarang) }}"
                       style="width: 100%; padding: 14px; border: 1.5px solid #e2e8f0; border-radius: 12px; outline: none; transition: 0.3s; font-size: 14px; box-sizing: border-box;"
                       onfocus="clearError(this)" 
                       onblur="validateField(this)">
            ...
            <div style="margin-bottom: 40px; background: #fdfdfd; padding: 25px; border-radius: 16px; border: 1px solid #f1f5f9;">
                ...
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 20px; background: #eff6ff; padding: 12px 18px; border-radius: 10px; border: 1px solid #dbeafe;">
                    ...
                    <div style="overflow: hidden;">
                        <p style="margin: 0; font-size: 11px; color: #64748b; text-transform: uppercase; font-weight: 700;">File Aktif:</p>
                        <p style="margin: 0; font-size: 13px; color: #1e40af; font-weight: 600; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">{{ basename($item->file_pdf) }}</p>
                    </div>
                </div>
                @endif

                <div style="position: relative; border: 2px dashed #cbd5e1; border-radius: 14px; background: white; padding: 30px; text-align: center; transition: 0.3s;" id="drop-area">
                    ...
                    <div id="file-preview">
                        <i class="fa-solid fa-cloud-arrow-up" style="font-size: 32px; color: #10b981; margin-bottom: 12px;"></i>
                        <p style="margin: 0; color: #475569; font-weight: 600; font-size: 14px;">Klik atau tarik file PDF baru di sini</p>
                        <p style="margin: 5px 0 0 0; color: #94a3b8; font-size: 12px;">
                            @if($item->file_pdf) Kosongkan jika tidak ingin mengganti file @else File PDF wajib diunggah @endif
                        </p>
                    </div>
                </div>
                <small id="err_file_pdf" style="display:none; color:#ef4444; font-size:12px; margin-top:8px;">
                    <i class="fa-solid fa-circle-exclamation"></i> <span id="err_file_pdf_msg">
                        @if(!$item->file_pdf) File PDF wajib diunggah. @endif
                    </span>
                </small>
            </div>
...
<script>
    // Apakah item sudah punya file PDF sebelumnya
    ...
    fileInput.addEventListener('change', function() {
        if (this.files && this.files[0]) {
            ...
            filePreview.innerHTML = `
                ...
                <p style="margin: 0; color: #10b981; font-weight: 700; font-size: 14px;">File Siap Diupload:</p>
