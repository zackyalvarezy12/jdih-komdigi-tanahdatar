...
<div style="padding: 40px; background-color: #f8fafc; min-height: 100vh; font-family: 'Inter', sans-serif;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            <h2 style="font-weight: 700; color: #1e293b; margin: 0; font-size: 26px; letter-spacing: -0.5px;">Edit Rancangan PUU</h2>
            <p style="color: #64748b; margin: 5px 0 0 0; font-size: 15px;">Perbarui draf rancangan peraturan perundang-undangan.</p>
        </div>
        <a href="{{ route('rancangan-puu.index') }}"
           style="text-decoration: none; color: #64748b; background: white; padding: 10px 22px; border-radius: 12px; border: 1px solid #e2e8f0; font-weight: 600; font-size: 14px; transition: all 0.3s ease; display: flex; align-items: center; gap: 8px;"
    ...
    <div style="background: white; border-radius: 20px; border: 1px solid #e2e8f0; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.04); overflow: hidden; max-width: 1000px; margin: 0 auto;">
        ...
        <div style="background: #f8fafc; padding: 20px 30px; border-bottom: 1px solid #e2e8f0;">
            <span style="background: #dcfce7; color: #166534; padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 700; text-transform: uppercase;">
                Form Perubahan Rancangan
            </span>
        </div>

        {{-- ✅ Action form pakai $encryptedId, bukan $item->id --}}
        <form id="mainForm"
              action="{{ route('rancangan-puu.update', $encryptedId) }}"
              method="POST"
            ...
            <div style="margin-bottom: 40px; background: #fdfdfd; padding: 25px; border-radius: 16px; border: 1px solid #f1f5f9;">
                ...
                @if($item->file_pdf)
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 20px; background: #eff6ff; padding: 12px 18px; border-radius: 10px; border: 1px solid #dbeafe;">
                    <i class="fa-solid fa-file-pdf" style="color: #3b82f6; font-size: 20px;"></i>
                    <div style="overflow: hidden;">
                        <p style="margin: 0; font-size: 11px; color: #64748b; text-transform: uppercase; font-weight: 700;">File Terpasang:</p>
                        <p style="margin: 0; font-size: 13px; color: #1e40af; font-weight: 600; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">
                            {{ basename($item->file_pdf) }}
                        </p>
                    </div>
                </div>
                @endif

                <div style="position: relative; border: 2px dashed #cbd5e1; border-radius: 14px; background: white; padding: 30px; text-align: center; transition: 0.3s;"
                     id="drop-area">
                    ...
                    <div id="file-preview">
                        <i class="fa-solid fa-file-arrow-up" style="font-size: 32px; color: #10b981; margin-bottom: 12px;"></i>
                        <p style="margin: 0; color: #475569; font-weight: 600; font-size: 14px;">Klik untuk mengganti draf PDF baru</p>
                        <p style="margin: 5px 0 0 0; color: #94a3b8; font-size: 12px;">
                            @if($item->file_pdf) Biarkan kosong jika tidak ingin mengubah file @else File PDF wajib diunggah @endif
                        </p>
                    </div>
                </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 15px; border-top: 1px solid #f1f5f9; padding-top: 30px;">
                <a href="{{ route('rancangan-puu.index') }}"
                   style="background: #f1f5f9; color: #475569; border: none; padding: 14px 30px; border-radius: 12px; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center;">
                    Batal
                </a>
                <button type="submit"
                        style="background: #10b981; color: white; border: none; padding: 14px 45px; border-radius: 12px; font-weight: 700; cursor: pointer; transition: all 0.3s; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.25); display: flex; align-items: center; gap: 10px;"
                        onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 15px rgba(16, 185, 129, 0.3)'"
                        onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(16, 185, 129, 0.25)'">
                    <i class="fa-solid fa-cloud-arrow-up"></i> Update Rancangan
                </button>
            </div>
        </form>
    </div>
    ...
    fileInput.addEventListener('change', function () {
        if (this.files && this.files[0]) {
            ...
            filePreview.innerHTML = `
                ...
                <p style="margin: 0; color: #10b981; font-weight: 700; font-size: 14px;">File Baru Siap:</p>
                <p style="margin: 5px 0 0 0; color: #334155; font-size: 13px; font-weight: 600;">${file.name}</p>
