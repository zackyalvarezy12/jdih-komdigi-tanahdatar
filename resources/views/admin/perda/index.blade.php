...
<div class="container-fluid" style="padding: 30px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            <h2 style="font-weight: 700; color: #1e293b; margin: 0;">Peraturan Daerah</h2>
            <p style="color: #64748b; margin-top: 5px;">Kelola data peraturan daerah Kabupaten secara sistematis.</p>
        </div>
        <a href="{{ route('perda.create') }}" class="btn" style="background: #10b981; color: white; border-radius: 12px; padding: 12px 24px; font-weight: 600; text-decoration: none;">
            <i class="fa-solid fa-plus" style="margin-right: 8px;"></i> Tambah Perda
        </a>
    </div>
    ...
    <div style="background: white; border-radius: 20px; padding: 25px; box-shadow: 0 10px 25px rgba(0,0,0,0.02); border: 1px solid #f1f5f9; overflow-x: auto;">
        <table style="width: 100%; border-collapse: collapse; min-width: 800px;">
            ...
            <tbody>
                ...
                @php $encId = Crypt::encrypt($perda->id); @endphp
                <tr style="border-bottom: 1px solid #f8fafc; transition: 0.3s;" onmouseover="this.style.backgroundColor='#fcfdfe'" onmouseout="this.style.backgroundColor='transparent'">
                    ...
                    <td style="padding: 20px;">
                        <div style="font-weight: 600; color: #334155; line-height: 1.5; font-size: 14px;">{{ $perda->judul }}</div>
                        @if($perda->slug)
                        <div style="margin-top:4px; font-size:11px; color:#94a3b8;">
                            <i class="fa-solid fa-link" style="font-size:10px;"></i> /perda/{{ $perda->slug }}
                        </div>
                        @endif
                    </td>
                    <td style="padding: 20px; text-align: center; color: #64748b; font-size: 14px;">
                        {{ $perda->nomor }} <span style="color: #cbd5e1;">/</span> {{ $perda->tahun }}
                    </td>
                    <td style="padding: 20px; text-align: center;">
                        <span style="background: {{ $perda->status == 'BERLAKU' ? '#ecfdf5' : '#fef2f2' }}; color: {{ $perda->status == 'BERLAKU' ? '#10b981' : '#ef4444' }}; padding: 6px 12px; border-radius: 8px; font-size: 11px; font-weight: 700; display: inline-block;">
                            {{ $perda->status }}
                        </span>
                    </td>
                    <td style="padding: 20px; text-align: center;">
                        @if($perda->file_pdf)
                            <a href="{{ asset('storage/' . $perda->file_pdf) }}" target="_blank" style="color: #6366f1; font-size: 20px;">
                    ...
                    <td style="padding: 20px; text-align: center;">
                        <div style="display: flex; gap: 8px; justify-content: center;">
                            {{-- Edit pakai encrypted ID --}}
                            <a href="{{ route('perda.edit', $encId) }}"
                            ...
                            {{-- Delete pakai encrypted ID --}}
                            <form action="{{ route('perda.destroy', $encId) }}" method="POST" onsubmit="return confirm('Hapus Perda \'{{ addslashes($perda->nomor) }}\'?')">
                                @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="6" style="padding: 50px; text-align: center; color: #94a3b8;">
                        ...
                        Belum ada data perda. <a href="{{ route('perda.create') }}" style="color: #10b981; font-weight: 700;">Tambah sekarang</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
